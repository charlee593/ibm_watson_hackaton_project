...
$(document).ready(function() {
  ...
  var conceptSuggestion = function(d) {
    ...
    var courseSuggestion = function(d) {
    if (getType(d.id) === 'concept') {
      return '<div><strong>' + d.label + '</strong> <i class=\'pull-right\'>' +
        getType(d.id) + '</i><br><i class="course-abstract">' + trunc(d.abstract) + '</i></div>';
    } else {
      return '<div><strong>' + d.label + '</strong> <i class=\'pull-right\'>' +
        "CSC Course" + '</i></div>';
    }
  ...
  $('.course--new-course-container').click(function(e) {
    e.preventDefault();
    var self = $(this);
    var concept = self.closest('.course');

    concept.find('.active').removeClass('active');
    concept.find('.course--input-container').addClass('active');
    concept.find('.course--input').focus();
  });
  /**
  ...
  $('.course--input').citypeahead({
    selectionCb: selectionCallbackCourse,
    hint: false
  }, {
    templates: {
    ...
    source: sourceLabelSearchCourse
  });

  $('.course--input').keyup(function (e) {
    console.log("Jelllooooo");
    var code = (e.keyCode ? e.keyCode : e.which);
    if (code == 13){
      console.log($('.tt-suggestion').first().attr('class'));
      $('.tt-suggestion').first().addClass('tt-cursor');
  ...
  function sourceLabelSearch(query, callback) {
    ...
    function sourceLabelSearchCourse(query, callback) {
    console.log(query + "000000000-==========");
    query_data = query;
    return $.get('/api/labelSearchCourse', {
      query: query,
      limit: 7,
      concept_fields: JSON.stringify({
        abstract: 1
      ...
      if(results.matches.length == 0){
        $('.tt-dataset').html('<div class="tt--search-hint"><i>no courses found</i></div>');
      }
  ...
  function selectionCallback(concept) {
    ...
    function selectionCallbackCourse(course) {
    var label = course.label;
    var $template = $('.course').last().clone();

    $template.find('.label').text(label);
    $template.find('.label').attr('course_id', course.id);
    $template.find('.course--close-icon').click(function() {
      $(this).closest('.course').remove();
      fetch_ted_based_on_concepts();
    });
    $template.insertBefore('.course:nth-last-child(1)');

    $('.course:nth-last-child(1) > .course--input-container').empty();
    $('.course:nth-last-child(1) > .course--input-container')
      .html('<input class="course--input" type="text">');
    ...
    $('.course:nth-last-child(1) > .course--input-container').removeClass('active');
    $('.course:nth-last-child(1) > .course--new-course-container').addClass('active');

    fetch_article_based_on_course();
  }
...
function fetch_ted_based_on_concepts() {
  ...
  console.log(input_concept_labels);
  ...
  unique_concept_array_g = [];
function fetch_article_based_on_course() {
  var course_array = [];
  var input_course_labels = [];
  var $user_input_course = $('.course--input-course-list > .course > .course--typed-course-container > .course--typed-course');
  for (var i = 0; i < ($user_input_course.length < 3 ? $user_input_course.length : 3); i++) {
    course_array.push($('.course--input-course-list > .course > .course--typed-course-container > .course--typed-course:eq(' + i + ')').attr('course_id'));
    input_course_labels.push($('.course--input-course-list > .course > .course--typed-course-container > .course--typed-course:eq(' + i + ')').text());
  }

  var parseText = course_array[course_array.length-1].split("/");
  console.log(parseText[parseText.length-1]);
  console.log(input_course_labels);


var doc = parseText[parseText.length-1];
console.log(doc);
//   $.get('/api/related_concepts', {
// document: "introduction_to_computer_programming",
// limit: 3
//     })
//     .done(function(results) {
...
//     });
  $.get('/api/testing', {
    ...
    .done(function(results) {
       ...
       // $('.course--abstract-course-list').empty();

        // var unique_concept_array = unique_concept_array_g;


        console.log(results.concepts.length);

        if (results.concepts.length)
          $('.course--abstract-course-title').addClass('active');

        for (var i = 0; i < results.concepts.length; i++) {
          if (check_duplicate_concept(unique_concept_array_g, results.concepts[i].concept.id) || unique_concept_array_g.length == 50)
            continue;
          else
            unique_concept_array_g.push(results.concepts[i].concept.id);

          var abstract_course_div = '<div class="course--abstract-course-list-container"><span class="course--abstract-course-list-item" course_id="' + results.concepts[i].concept.id + '">' + results.concepts[i].concept.label + '</span></div>';
          $('.course--abstract-course-list').append(abstract_course_div);
        }
    });
  ...
  $.get('/api/conceptualSearch', {
    ...
    .done(function(results) {
      ...
      $('#TED-panel-list').empty();
      for (var i = 0; i < results.results.length; i++){
                  generate_TED_panel(results.results[i], input_course_labels);
                  // generate_research_panel(results.results[i], input_course_labels);
      }
    ...
    $.get('/api/conceptualSearchResearch', {
      ids: course_array,
      limit: 5,
      document_fields: JSON.stringify({
        user_fields: 1
...
function generate_TED_panel(TED_data, your_input_concepts) {
  console.log("323423fsdf===========");
  console.log(your_input_concepts);
  console.log(TED_data);
  var TED_panel = '<div class="_TED-panel">' + '<div class="_TED-panel--TED">';

  var TED_info_above = '<div class="TED--info-above">' + '<a class="TED--title" href="' + TED_data.user_fields.url + '" target="_blank">' + TED_data.user_fields.title + '</a>' + '<div class="TED--score">' + '<span class="TED--score-title">' + 'Confidence Score:' + '</span>' + '<span class="TED--score-value">' + Math.floor(TED_data.score * 100) + '</span>' + '</div>' + '</div>';
  ...
  var TED_info_below = '<div class="TED--info-below">' + '<a class="TED--title" href="' + TED_data.user_fields.url + '" target="_blank">' + TED_data.user_fields.title + '</a>' + '<div class="TED--score">' + '<span class="TED--score-title">' + 'Confidence Score:' + '</span>' + '<span class="TED--score-value">' + Math.floor(TED_data.score * 100) + '%' + '</span>' + '</div>' + '</div>';
  ...
  function generate_research_panel(TED_data, your_input_concepts) {
  console.log("323423fsdf===========");
  console.log(your_input_concepts);
  console.log(TED_data);
  var TED_panel = '<div class="_TED-panel">' + '<div class="_TED-panel--TED">';
  ...
  $('#research-panel-list').append(TED_panel);
